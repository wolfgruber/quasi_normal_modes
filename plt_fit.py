...
def PlotLog(rfix): # plotting the log(abs(phi)) at r = rfix
...
def PlotLog3(rfix): # plotting the log(abs(phi)) at r = rfix for three different phi
...
def PlotMovie3(t): # plotting 3 different phi animated
    plt.plot(rp, phip1[t,:])
    plt.plot(rp, phip2[t,:])
    plt.plot(rp, phip3[t,:])
    plt.ylim(-2,2)
...
'''def export(phi, t, r, name):
    dr = r[1] - r[0]
    dt = t[1] - t[0]
    R = r[-1]
    T = t[-1]
    np.savetxt("phi/phi_"+name+".txt", phi)
    np.savetxt("phi/t_"+name+".txt", t)
    np.savetxt("phi/r_"+name+".txt", r)
    ...
    plt.subplot(222)
    plt.plot(r, phi[int(T/(2*dt)),:])
    plt.title("@t = "+str(int(T/2)))
    plt.xlim(0,np.max(T))
    
    plt.subplot(223)
    plt.plot(t, np.log(np.abs(phi[:,0])))
    plt.title("@r = 0")
    plt.xlim(0,np.max(T))
    
    plt.subplot(224)
    plt.plot(t, np.log(np.abs(phi[:,int(R*0.5/dr)])))
    plt.title("@r = "+str(R*0.5))
    plt.xlim(0,np.max(T))
    
    plt.savefig("phi/phi_"+name+".jpg")
    plt.close()
    
def imp(name):
    phi = np.loadtxt("phi/phi_"+name+".txt")
    t = np.loadtxt("phi/t_"+name+".txt")
    r = np.loadtxt("phi/r_"+name+".txt")
